...
public class ClienteBean implements Serializable {
    ...
    public void init() {
        ...
        if (idParam != null && !idParam.isEmpty()) {
            // Está em modo edição
            this.edicaoMode = true;

            // Aqui você pode carregar o cliente por ID (via REST, etc.)
            carregarClientePorId(Long.parseLong(idParam));
        }
    ...
    private void carregarClientePorId(Long id) {
        try {
            ...
            if (conexao.getResponseCode() == 200) {
                try (BufferedReader br = new BufferedReader(new InputStreamReader(conexao.getInputStream(), StandardCharsets.UTF_8))) {
                    ...
                    this.cliente = mapper.readValue(resposta.toString(), Cliente.class);
    ...
    public String getTextoBotaoSalvar() {
        return edicaoMode ? "Atualizar Cliente" : "Salvar";
    }
